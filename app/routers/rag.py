...
@limiter.limit("10/minute")
def ask_document_stream(request: Request, payload: AskDocumentRequest, user_id: int = Depends(get_current_user_id)):
    chunks = search_similar(user_id=user_id, query=payload.question, document_id=payload.document_id)
